...
# Menulis judul
st.markdown("<h1 style='text-align: center; '> Prediksi Nilai Kalor Unloading</h1>", unsafe_allow_html=True)
st.markdown("<h1 style='text-align: center; '>PLN Indonesia Power JPR PGU</h1>", unsafe_allow_html=True)

# Pilihan utama
pilihan = st.sidebar.selectbox('Penasaran kan mau liat hasil prediksinya ?', ('Prediksi NK Unloading', 'Prediksi NK Lab'))

if pilihan == 'Prediksi NK Unloading':
    ...
    # Load model
    my_model1 = pickle.load(open('nkunload.pkl', 'rb'))

    # Predict
    result = round(float(my_model1.predict(df_final)), 2)

    st.write("<center><b><h4>Prediksi Nilai Kalor Unloading =", result, "</b></h4>Kcal/Kg", unsafe_allow_html=True)
